 ...
 * 3. **Replace dynamic ID segments** — v4 UUIDs and 24-character hex MongoDB
 *    ObjectIDs found anywhere in the remaining path are replaced with `:id`.
...
export function normalizeRouteState(url: string): string {
  ...
  path = path.replace(UUID_V4_RE, ':id').replace(MONGO_ID_RE, ':id');
