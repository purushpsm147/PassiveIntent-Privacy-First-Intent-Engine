    ...
    <>
      <div className="demo-header">
        ...
        <p className="demo-description">
          ...
          sensitive routes in production. The standalone <strong>MarkovGraph</strong> class lets
          you build, serialize, and restore graphs independently.
        </p>
      </div>
            ...
            <tbody>
              {predictions.map(({ state, probability }) => (
                <tr key={state}>
                  ...
                  <td>
                    <div className="progress-track" style={{ width: 140, height: 6, display: 'inline-block', borderRadius: 3, overflow: 'hidden', background: 'var(--bg-3)' }}>
                      <div className="prob-fill" style={{ width: `${Math.round(probability * 100)}%`, height: '100%' }} />
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <div className="card">
        ...
        <div className="btn-row">
          <button className="btn btn-secondary" onClick={buildStandaloneGraph}>Build sample graph</button>
          <button className="btn btn-primary"   onClick={serializeJSON}        disabled={!graphBuilt}>Serialize JSON</button>
          <button className="btn btn-ghost"     onClick={binarySize}           disabled={!graphBuilt}>Binary vs JSON size</button>
        </div>
        ...
        {binaryInfo && (
          <div className="alert alert-info" style={{ marginTop: 10 }}>{binaryInfo}</div>
        )}
        {serialized && (
          <div className="code-block" style={{ marginTop: 10 }}>
            <div className="code-label">MarkovGraph.toJSON()</div>
            <pre style={{ padding: 16, fontFamily: 'var(--font-mono)', fontSize: 12, lineHeight: 1.5, color: 'var(--text)' }}>{serialized}</pre>
          </div>
        )}
      </div>
