...
export default function DwellTime() {
  ...
  const [status, setStatus] = useState<{ type: 'success'|'warning'|'info'; msg: string } | null>(null);
  ...
  function buildBaseline() {
    ...
    setStatus({ type: 'success', msg: 'Baseline built: 10 quick visits avg ~3s on /checkout/payment. Now simulate hesitation →' });
  }
  ...
  function simulateHesitation() {
    ...
    setStatus({ type: 'warning', msg: 'Simulated 5-minute dwell on /checkout/payment. Check the event log for dwell_time_anomaly.' });
  }
    ...
    <>
      <div className="demo-header">
        ...
        <p className="demo-description">
          Uses <strong>Welford's online algorithm</strong> to maintain running mean/variance
          per state — no raw timestamps are ever stored. When the z-score of the current
          dwell exceeds the threshold, the event fires. The controllable timer adapter lets
          you simulate hours of dwell in a click.
        </p>
      </div>

      <div className="card">
        <div className="card-title">Simulate hesitation</div>
        <ol style={{ color: 'var(--text-muted)', fontSize: 13, lineHeight: 1.9, paddingLeft: 20, marginBottom: 14 }}>
        ...
        <div className="btn-row">
          ...
          <button className="btn btn-ghost" onClick={resetTimer}>↩ Reset Timer</button>
        </div>
